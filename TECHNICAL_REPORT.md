...
### Evidence 5: Visual Evidence & Live Application Screenshots

The following high-resolution screenshots were captured directly from the live, fully integrated system running locally (`http://localhost:3000` connected to `http://localhost:5000`). They provide concrete visual proof that every subsystem, database model, geospatial pipeline, and user interface operates seamlessly:

#### Figure 1: Identity Access & Authentication Portal
![Figure 1: Identity Access & Authentication Portal](./docs/screenshots/01_signin_page.png)
*Figure 1 demonstrates the responsive glassmorphism login portal. It validates user credentials, handles session tokens, and interfaces with the dual-flow authentication backend.*

---

#### Figure 2: Satellite Analysis & Monitoring Workspace
![Figure 2: Satellite Analysis & Monitoring Workspace](./docs/screenshots/03_dashboard_analysis.png)
*Figure 2 displays the primary monitoring dashboard. Notice the active session for **Bilal Farid** in the upper right header, live aggregate metrics (1,249 analyses, 520.8 km² monitored), configurable bi-temporal date selectors (2020 to 2024), and the interactive Leaflet GIS map centered on the Margalla Hills AOI (33.7438° N, 73.0228° E).*

---

#### Figure 3: Live Bi-Temporal Detection, Visual Drift & Forest Health Metrics
![Figure 3: Live Bi-Temporal Detection, Visual Drift & Forest Health Metrics](./docs/screenshots/03b_live_scan_detection_result.png)
*Figure 3 illustrates the live detection results generated by the Python spectral engine upon clicking **"Run Analysis"**. It presents the $T_1$ baseline visual drift (dense canopy), the $T_2$ current state (clearing and logging roads), the Final Delta overlay highlighting deforestation zones in crimson, and computed forest health metrics (94% healthy, 6% deforested with temporal trend analytics).*

---

#### Figure 4: Historical Scans & Audit Records
![Figure 4: Historical Scans & Audit Records](./docs/screenshots/04_history_reports.png)
*Figure 4 shows the persistent analysis history interface. Scans saved from the dashboard are stored in the database via `/api/reports` and can be re-inspected or exported as formal PDF audit certificates.*

---

#### Figure 5: Environmental Intelligence & Multi-Spectral News Portal
![Figure 5: Environmental Intelligence & Multi-Spectral News Portal](./docs/screenshots/05_news_portal.png)
*Figure 5 highlights the curated news and remote sensing discoveries portal. It features multi-criteria search, category filters (Satellite Monitoring, Climate Science, Policy & Action), and real-time news synchronization.*

---

#### Figure 6: Forest Watchdog Community Hub & Discussion Portal
![Figure 6: Forest Watchdog Community Hub & Discussion Portal](./docs/screenshots/06_community_hub.png)
*Figure 6 showcases the community watchdog hub where field rangers, conservationists, and citizen scientists can publish ground observations, dispute automated classifications, and upvote critical reports.*

---

#### Figure 7: Official Environmental Authority Complaint Dispatcher
![Figure 7: Official Environmental Authority Complaint Dispatcher](./docs/screenshots/07_contact_complaint.png)
*Figure 7 presents the formal legal alert dispatch interface. Critical deforestation incidents identified on the dashboard can be attached directly to automated notices dispatched to the Ministry of Climate Change and provincial forest authorities.*
